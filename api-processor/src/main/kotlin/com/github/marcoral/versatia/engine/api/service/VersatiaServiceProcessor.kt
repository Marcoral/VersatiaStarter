...
class VersatiaServiceProcessorProvider : SymbolProcessorProvider {
    override fun create(environment: SymbolProcessorEnvironment) = VersatiaServiceProcessor(environment.logger, environment.codeGenerator)
}

class VersatiaServiceProcessor(private val logger: KSPLogger, private val codeGenerator: CodeGenerator): SymbolProcessor {
    override fun process(resolver: Resolver): List<KSAnnotated> = resolver.processAnnotated(VersatiaService::class) {
        forEach { it.accept(Visitor(), Unit) }
    }

    private inner class Visitor : KSVisitorVoid() {
        override fun visitClassDeclaration(classDeclaration: KSClassDeclaration, data: Unit) {
            val annotation = classDeclaration.getAnnotationsDeclarationsByType(VersatiaService::class).first()
